		...
		do {
			...
			try {
				...
				case 2:
				    ...
				    String marcaBuscar = datos.nextLine();
				    boolean encontrado = false;

				    for (Carros c : imp.mazda) {
				        if (c.getMarca().equalsIgnoreCase(marcaBuscar)) {
				            System.out.println("Auto encontrado: " + c);
				            encontrado = true;
				        }
				    }
				...
				case 4:
				    if (imp.mazda.size() == 0) {
				        ...
				        System.out.print("Ingresa la marca del auto que deseas eliminar: ");
				        marca = datos.nextLine();

				        boolean eliminado = false;

				        for (int i = 0; i < imp.mazda.size(); i++) {
				            if (imp.mazda.get(i).getMarca().equalsIgnoreCase(marca)) {
				                imp.mazda.remove(i);
				                eliminado = true;
				                System.out.println("Auto con marca '" + marca + "' eliminado correctamente.");
				                break; 
				            }
				        }
